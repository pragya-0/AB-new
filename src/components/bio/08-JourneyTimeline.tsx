  ...
  return (
    ...
    >
      <div className="relative flex min-h-[160px] items-center border-b border-white/10 bg-white/[0.035] p-6 lg:min-h-[390px] lg:border-b-0 lg:border-r">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.32em] text-[#58a0ff]">
            Chapter {String(index + 1).padStart(2, "0")}
          </p>

          <p className="mt-4 text-[58px] font-black leading-none tracking-[-0.09em] text-white md:text-[76px] lg:text-[82px]">
            {item.year}
          </p>
        </div>
      ...
      <div className="flex min-h-[320px] flex-col justify-center p-7 md:min-h-[390px] md:p-10 lg:p-12">
        <p className="mb-5 text-[10px] font-black uppercase tracking-[0.35em] text-[#58a0ff]">
          {item.tag}
        </p>

        <h3 className="max-w-[560px] text-[36px] font-black leading-[0.92] tracking-[-0.06em] text-white md:text-[48px]">
          {item.title}
        </h3>
        ...
        <div className="mt-7 flex flex-wrap gap-2.5">
          {item.chips.map((chip) => (
            <span
              key={chip}
              className="rounded-full border border-white/12 bg-white/[0.06] px-4 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-white/70"
            >
              {chip}
            </span>
          ))}
...
export default function JourneyTimeline() {
  return (
    <BioSectionShell
      ...
      title="29 Years Across Technology, Creativity & Global Innovation"
